    ...
    public partial class Year2022
    {
        public class Day08 : DayBase
        {
            ...
            public override string Solve(bool Part1)
            {
                ...
                //parse all
                _heights = InputData.Select(y => y.Select(l => int.Parse(l.ToString())).ToArray()).ToArray();
                if (Part1 && !Verbose) PrintIntGrid(_heights, 1);
                ...
                if (Part1)
                {
                    if (!Verbose) PrintBoolGrid(_visibleTrees);
                    return FormatResult(_visibleTrees.Select(y => y.Count(x => x == true)).Sum(), "visible trees");
                }
                if (!Verbose) PrintIntGrid(_scenicScores, 3);
            ...
            void PrintBoolGrid(bool[][] Grid)
            {
                ...
                Grid.ToList().ForEach(y => Console.WriteLine(y.Select(x => x ? 'x' : '.').ToArray()));
                //for (int y = 0; y < Grid.Length; y++) Console.WriteLine(Grid[y].Select(x => x ? 'x' : '.').ToArray());
                Console.WriteLine("");
            }
            ...
            {
                //for (int y = 0; y < Grid.Length; y++) Console.WriteLine(string.Join(' ', Grid[y].Select(x => x.ToString() + " ".Repeat(digits).Substring(0, digits)).ToArray()));
                Grid.ToList().ForEach(y => Console.WriteLine(string.Join(' ', y.Select(x => x.ToString() + " ".Repeat(digits).Substring(0, digits)).ToArray())));
                Console.WriteLine("");
            }
        }
    }
