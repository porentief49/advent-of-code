    ...
    public partial class Year2023 {
        ...
        public class Day15 : DayBase {
            ...
            public override string Solve() {
                ...
                Box[] boxes = Enumerable.Range(0, boxCount).Select(i => new Box()).ToArray();
                ...
                int total = 0;
                for (int b = 0; b < boxCount; b++) for (int l = 0; l < boxes[b].Lenses.Count; l++) total += ((b + 1) * (l + 1) * int.Parse(boxes[b].Lenses[l].focal));
                return total.ToString();
            }

            private static int CalcHash(string value) => value.Select(v => (int)v).Aggregate(0, (x, y) => ((x + y) * 17) % boxCount);
